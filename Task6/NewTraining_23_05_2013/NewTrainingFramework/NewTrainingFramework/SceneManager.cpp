...
int SceneManager::readFile(const char * filePath)
{
	...
	else
	{
			...
			if (m_objAmount > 0)
			{
				...
				for (int indexObj = 0; indexObj < m_objAmount; indexObj ++)
				{
					...
					if (fscanf_s(pFile, "NUM_CUBE_TEXTURES %d\n", &tmpCubeTexAmount) != EOF)
					{
						...
						for (int indexCubeTex = 0; indexCubeTex < tmpTexAmount; indexCubeTex++)
						{
							//Lack of implementation for cube textures
						}
					}
					...
					float tmpPosX, tmpPosY, tmpPosZ, tmpRotX, tmpRotY, tmpRotZ, tmpSclX, tmpSclY, tmpSclZ, tmpTiltFac;
					if (fscanf_s(pFile, "POSITION %f, %f, %f\nROTATION %f, %f, %f\nSCALE %f, %f, %f\nTILING_FACTOR %f\n", 
								&tmpPosX, &tmpPosY, &tmpPosZ, &tmpRotX, &tmpRotY, &tmpRotZ, &tmpSclX, &tmpSclY, &tmpSclZ, &tmpTiltFac) != EOF)
					{
						//Lack of tilt factor implementation
						//m_objPtr[indexObj].m_position.SetTranslation(tmpPosX, tmpPosY, tmpPosZ);
						//m_objPtr[indexObj].m_rotationX.SetRotationX(tmpRotX);
						//m_objPtr[indexObj].m_rotationY.SetRotationY(tmpRotY);
						//m_objPtr[indexObj].m_rotationZ.SetRotationZ(tmpRotZ);
						//m_objPtr[indexObj].m_scale.SetScale(tmpSclX, tmpSclY, tmpSclZ);	
						m_objPtr[indexObj].set(tmpPosX, tmpPosY, tmpPosZ, tmpRotX,tmpRotY, tmpRotZ, tmpSclX, tmpSclY, tmpSclZ);
					}
				}
			}
		...
		float tmpFogColorX, tmpFogColorY, tmpFogColorZ, tmpFogStart, tmpFogRange;
		if (fscanf_s(pFile, "#FOG\nFOG_COLOR %f, %f, %f\nFOG_START %f\nFOG_RANGE %f\n",
					&tmpFogColorX, &tmpFogColorY, &tmpFogColorZ, &tmpFogStart, &tmpFogRange) != EOF)
		{
