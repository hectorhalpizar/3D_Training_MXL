...
int SceneManager::readFile(const char * filePath)
{
	...
	else
	{
			...
			if (m_objAmount > 0)
			{
				...
				for (int indexObj = 0; indexObj < m_objAmount; indexObj ++)
				{
					...
					if (fscanf_s(pFile, "SHADER %d\n", &tmpShader) != EOF)
					{
						...
						if (tmpShader == 6)
							Log::GetInstance()->printMessage(TAG, "Breakpoint parsing!");
					...
					if (fscanf_s(pFile, "WATER_ID %d\n", &tmpWaterId) != EOF)
					{
						m_objPtr[indexObj].waterId = tmpWaterId;						
					}
		...
		//Parsing WATER
		Log::GetInstance()->printMessage(TAG, "Parsing WATER");
		int tmpWatersAmout;
		if (fscanf_s(pFile, "#WATER: %d\n", &tmpWatersAmout) != EOF)
		{
			Log::GetInstance()->printMessage(TAG, "Parsing water objects");

			setWaterAmount(tmpWatersAmout);

			if (getWaterAmount() > 0)
			{
				m_waters = new Water[getWaterAmount()];

				for (int indexWater = 0; indexWater < getWaterAmount(); indexWater ++)
				{
					int tmpId;
					float tmpWaterR, tmpWaterG, tmpWaterB, tmpFresnelPow, tmpDepthAjust;
					if (fscanf_s(pFile, "ID %d\nWATER_COLOR %f, %f, %f\nFRESNELPOWER %f\nDEPTHADJUST %f\n", &tmpId, &tmpWaterR, &tmpWaterG, &tmpWaterB, &tmpFresnelPow, &tmpDepthAjust) != EOF)
					{
						m_waters[indexWater].ID = tmpId;
						m_waters[indexWater].color.x = tmpWaterR;
						m_waters[indexWater].color.y = tmpWaterG;
						m_waters[indexWater].color.z = tmpWaterB;
						m_waters[indexWater].fresnel_power = tmpFresnelPow;
						m_waters[indexWater].depth_ajust = tmpDepthAjust;
					}
				}
			}
...
void SceneManager::Update(float deltaTime)
{
	...
	for (int indexObj = 0; indexObj < m_objAmount; indexObj++)
	{
		...
		if ((m_objPtr[indexObj].m_shaderPtr->getID() == 5 || m_objPtr[indexObj].m_shaderPtr->getID() == 6 || m_objPtr[indexObj].m_shaderPtr->getID() == 7)
...
void SceneManager::setWaterAmount(int value)
{
	m_watersAmount = value;
}

int SceneManager::getWaterAmount()
{
	return m_watersAmount;
}
...
Water * SceneManager::getWater(int byId)
{
	Water * tmpWater = NULL;
	if (byId > -1)
	{
		for (int indexWater = 0; indexWater < getWaterAmount(); indexWater++)
		{
			if (m_lights[indexWater].ID == byId)
			{
				tmpWater = &m_waters[indexWater];
				break;
			}
		}
	}
