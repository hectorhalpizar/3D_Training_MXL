...
void Object::Update(float deltaTime)
{
	...
	if (m_shaderPtr->getID() == 5 || m_shaderPtr->getID() == 6 || m_shaderPtr->getID() == 7)
	{
		if (light_color == NULL)
		{
			if ( m_shaderPtr->getID() == 7)
...
void Object::Draw()
{
	...
	if(m_shaderPtr->tangentAttribute != -1)
	{
		glEnableVertexAttribArray(m_shaderPtr->tangentAttribute);
		glVertexAttribPointer(m_shaderPtr->tangentAttribute, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)(13 * sizeof(float)));	
	}

	if(m_shaderPtr->binormalAttribute != -1)
	{
		glEnableVertexAttribArray(m_shaderPtr->binormalAttribute);
		glVertexAttribPointer(m_shaderPtr->binormalAttribute, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)(10 * sizeof(float)));	
	}
	...
	if (m_shaderPtr->uniformFresnelPower != -1)
	{
		Log::GetInstance()->printMessage(TAG, "fresnel_power [%f]", SceneManager::GetInstance()->getWater(waterId)->fresnel_power);
		glUniform1f(m_shaderPtr->uniformFresnelPower, SceneManager::GetInstance()->getWater(waterId)->fresnel_power);
	}

	if (m_shaderPtr->uniformDepthAdjust != -1)
	{
		Log::GetInstance()->printMessage(TAG, "depth_ajust [%f]", SceneManager::GetInstance()->getWater(waterId)->depth_ajust);
		glUniform1f(m_shaderPtr->uniformDepthAdjust, SceneManager::GetInstance()->getWater(waterId)->depth_ajust);
	}
	...
	//	if (m_shaderPtr->uniformPosDir >= 0)
		if (m_shaderPtr->uniformPosDir != -1)
		{
			...
			//if (m_shaderPtr->getID() == 6)
				glUniform3fv(m_shaderPtr->uniformPosDir, getLightAmount(), &light_PosDir[0].x);
			/*else if (m_shaderPtr->getID() == 7)
				glUniform3fv(m_shaderPtr->uniformPosDir, 1, &light_PosDir[0].x);*/
		}
	...
	//	if (m_shaderPtr->uniformLightColor >= 0)
		if (m_shaderPtr->uniformLightColor != -1)
		{
			...
			//else if (m_shaderPtr->getID() == 7)
				//glUniform3fv(m_shaderPtr->uniformLightColor, 1, &light_color[0].x);
		}
	}
